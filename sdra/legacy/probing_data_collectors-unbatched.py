        ...
        with torch.no_grad():
            if isinstance(self.model, prefixtuning.Model):
                ...
                encoder_outputs = self.model.pretrain_model.encoder(
                    input_ids=torch.LongTensor(tokenized_txt.data['input_ids']).to(self.device_name),
                    attention_mask=torch.LongTensor(tokenized_txt.data['attention_mask']).to(self.device_name),
