...
def precompute_wikisql_uskg_schemas_dict(orig_tables_path, db_path):
...
def uskg_sample_to_struct_input_spider(uskg_sample, uskg_schemas_dict):
    """ Build the struct_in for a uskg_sample. """
    db_id = uskg_sample["db_id"]
    uskg_schema = uskg_schemas_dict[db_id]
    
    return serialize_schema_spider(
...
def uskg_sample_to_struct_input_wikisql(uskg_sample, uskg_schemas_dict):
    """ Build the struct_in for a uskg_sample. """
    db_id = uskg_sample["table_id"]

    return serialize_schema_wikisql(
        question=uskg_sample["question"],
        db_path=uskg_schema["db_path"],
        db_id=db_id,
        db_column_names=uskg_schema["db_column_names"],
...
def collect_node_char_ranges(sample, tokenizer=None, tokenizer_args=None, txt=None, tokenized_txt=None, uskg_schemas_dict=None, debug=False):
    ...
    if 'txt_pieces' in sample:
        ...
        struct_in = uskg_sample_to_struct_input_spider(sample, uskg_schemas_dict)
