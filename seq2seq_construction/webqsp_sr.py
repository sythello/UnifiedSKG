...
def kgqa_get_input(question: str, kg_tuples: list, entities: list) -> str:
    ...
    serialized_entity = " ".join([": ".join(elm) for elm in entities]).strip()  # elm = (text, kb_id)
...
class TrainDataset(Dataset):
    ...
    def __init__(self, args, raw_datasets, cache_root):
        ...
        cache_path = os.path.join(cache_root, 'webqsp_SR_train.cache')
        if os.path.exists(cache_path) and args.dataset.use_cache:
            ...
            for raw_data in tqdm(self.raw_datasets):
                ...
                kg_tuples = extend_data["subgraph_tuples"]
                ...
                seq_out = ', '.join([ans[1] for ans in answers])      # ans=(text,kb_id); same way as USKG-CWQ; use kb_id since text not available in SR
...
class DevDataset(Dataset):
    ...
    def __init__(self, args, raw_datasets, cache_root):
        ...
        cache_path = os.path.join(cache_root, 'webqsp_SR_dev.cache')
        if os.path.exists(cache_path) and args.dataset.use_cache:
            self.data = torch.load(cache_path)
        else:
            self.data = []
...
class TestDataset(Dataset):
    ...
    def __init__(self, args, raw_datasets, cache_root):
        ...
        cache_path = os.path.join(cache_root, 'webqsp_SR_test.cache')
        if os.path.exists(cache_path) and args.dataset.use_cache:
            self.data = torch.load(cache_path)
        else:
            self.data = []
